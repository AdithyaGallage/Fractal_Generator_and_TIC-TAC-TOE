...
class JuliaPoint {
    ...
    private boolean julia; // identify point is a julia or not
    ...
    static double c_zX; // real part of the constant
    static double c_zY; // imaginary part of the constant
    static int iterations; // number of iterations

    //Setters
    public void setN(int n){ this.n = n; }
    public void setJulia(boolean julia){ this.julia = julia; }
    ...
    public boolean getJulia(){ return this.julia; }
    ...
    //Constructor
    JuliaPoint(int x, int y, double zX, double zY){
        this.x = x;
        this.y = y;
        this.zX = zX;
        this.zY = zY;
    ...
    // This method use to calculate the value of n and identify the point is in julia set or not
    public void isJulia(){
        // initialize Z0
        double zRe = this.zX;
        double zIm = this.zY;
        // iterate Z1 = Z0^2 + C for given number of iterations 
        for(int i = 0; i <= JuliaPoint.iterations; i++){
            ...
            Complex z_next = Complex.addComplex(JuliaPoint.c_zX, JuliaPoint.c_zY, z_square.x, z_square.y);
            // Initialize Z0 for next iteration
            zRe = z_next.x;
            zIm = z_next.y;
            /* If the square of the magnitude is greater than 4
               n is set as i and identify that complex number in julia set*/  
            if(magnitude > 4){
                this.n = i;
                this.julia = false;
                return;
           ...
           that complex number is not in the julia set*/
            this.n = JuliaPoint.iterations;
            this.julia = true;    
        }
    }
    // This method use to set the colour of the pixels
    public void setRGB(){
        // If pixel is not in the julia set colour set according to  the n
        if(!this.julia){
